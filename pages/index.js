...
export default function Home( { countries } ) {
  return (
    <div className={styles.container}>
      <Head>
        <title>Countries for JW</title>
        <meta name="description" content="countries for JW the explorer to visit" />
    ...
    <div className={styles.grid}>
      {countries.map((country) => (
        <div key={country.code} className={styles.card}>
          ...
          <p>
            {country.code} - {country.emoji} located in {country.continent.name}
          </p>
        </div>
      ))}
    </div>
      ...
      <footer className={styles.footer}>
        ...
        >
          ...
          <span className={styles.logo}>
            <Image src="/flying-gg.svg" alt="flying-GG Logo" width={150} height={100} />
          </span>
        </a>
      </footer>
    </div>
...
export async function getStaticProps() {
  ...
  return {
    props: {
      countries: data.countries.slice(0, 16),
    },
 };
}